...
flyctl status
echo ""
# Get app name for URLs
APP_NAME=$(flyctl status -j 2>/dev/null | jq -r .Name 2>/dev/null)
if [ -n "$APP_NAME" ] && [ "$APP_NAME" != "null" ]; then
    echo "🔗 Your app URL: https://$APP_NAME.fly.dev"
    echo "📱 Configure your Twilio webhook to: https://$APP_NAME.fly.dev/twilio/sms"
else
    echo "🔗 Your app URL: https://[your-app-name].fly.dev"
    echo "📱 Configure your Twilio webhook to: https://[your-app-name].fly.dev/twilio/sms"
    echo "⚠️  Could not automatically determine app name. Check 'flyctl status' for the correct URL."
fi
